# EchoProof, phase plan

Phases run in order. They are sequenced by dependency, not by date. A phase starts when the phase before it is
...
and before credentials are collected.
...
which is git ignored, and verify the real model ID against the provider's
/v1/models rather than guessing it. DEEPGRAM_API_KEY is added at Phase 2, not earlier.
